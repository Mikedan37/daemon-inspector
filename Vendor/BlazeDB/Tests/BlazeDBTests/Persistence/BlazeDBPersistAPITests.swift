...
final class BlazeDBPersistAPITests: XCTestCase {
    ...
    func testPersistForcesMetadataFlush() throws {
        ...
        do {
            let db = try BlazeDBClient(name: "Test", fileURL: tempURL, password: "BlazeDBPersistAPITest123!")
        ...
        let db2 = try BlazeDBClient(name: "Test", fileURL: tempURL, password: "BlazeDBPersistAPITest123!")
    ...
    func testPersistBeforeCriticalOperation() throws {
        ...
        var db: BlazeDBClient? = try BlazeDBClient(name: "Test", fileURL: tempURL, password: "BlazeDBPersistAPITest123!")
        ...
        let dbBackup = try BlazeDBClient(name: "Backup", fileURL: backupURL, password: "BlazeDBPersistAPITest123!")
    ...
    func testPersistDoesNotThrowOnNormalOperation() throws {
        ...
        // CRITICAL: Use loadSecure() since the database saves SecureLayout, not plain StorageLayout
        let password = "BlazeDBPersistAPITest123!"
        guard let salt = "AshPileSalt".data(using: .utf8) else {
            XCTFail("Failed to create salt")
            return
        }
        let signingKey = try KeyManager.getKey(from: password, salt: salt)
        let layout = try? StorageLayout.loadSecure(from: metaURL, signingKey: signingKey, password: password, salt: salt)
    ...
    func testAutomaticFlushAt100Operations() throws {
        ...
        // Use a scope to ensure db is deinitialized before reopening
        do {
            let db = try BlazeDBClient(name: "Test", fileURL: tempURL, password: "BlazeDBPersistAPITest123!")
            
            // Insert exactly 100 records (auto-save disabled, but deinit will save)
            for i in 0..<100 {
                _ = try db.insert(BlazeDataRecord(["index": .int(i)]))
            }
            
            // NO explicit persist() call - rely on deinit
        } // db goes out of scope here, deinit saves
        
        // Reopen immediately - should see all 100
        let db2 = try BlazeDBClient(name: "Test", fileURL: tempURL, password: "BlazeDBPersistAPITest123!")
        let records = try db2.fetchAll()
        
        XCTAssertEqual(records.count, 100, "Deinit should save all records")
    }
