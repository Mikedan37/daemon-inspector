...
final class CodableIntegrationTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            db = try BlazeDBClient(
                name: "CodableTest",
                fileURL: tempURL,
                password: "CodableIntegrationTest123!"
            )
        } catch {
    ...
    func testCodableVsDynamicPerformance() throws {
        ...
        // Log performance comparison (informational only)
        let ratio = codableTime / dynamicTime
        print("Codable: \(codableTime * 1000)ms, Dynamic: \(dynamicTime * 1000)ms (ratio: \(String(format: "%.2f", ratio))x)")
        
        // Sanity check: both should complete in reasonable time (not a strict performance assertion)
        XCTAssertLessThan(codableTime, 1.0, "Codable insertMany took too long")
        XCTAssertLessThan(dynamicTime, 1.0, "Dynamic insertMany took too long")
    }
