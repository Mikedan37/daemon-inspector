...
final class ArrayDictionaryEdgeTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            db = try BlazeDBClient(name: "ArrayDictTest", fileURL: tempURL, password: "ArrayDictionaryEdge123!")
        } catch {
    ...
    /// Test that arrays/dictionaries are skipped correctly in compound indexes
    func testArrayDictionaryInCompoundIndexFallback() throws {
        print("📦 Testing array/dict fallback in compound indexes...")
        
        let collection = db.collection
        
        // Create a simple index on status (since arrays can't be indexed)
        // Note: Compound indexes skip array fields, so a compound index on ["status", "tags"]
        // would only index "status" when tags is an array
        try collection.createIndex(on: ["status"])
        
        _ = try db.insert(BlazeDataRecord([
            "status": .string("active"),
            "tags": .array([.string("urgent"), .string("bug")]),  // Array is skipped in index
            "title": .string("Test")
        ]))
        
        print("  Inserted record with array in compound index field")
        
        // Arrays are skipped in compound indexes, so query only by status
        // The index key will only contain ["active"], not ["active", ""]
        let results = try collection.fetch(byIndexedFields: ["status"], 
                                          values: ["active"])
        
        // Should find the record (array was skipped, so index key is just ["active"])
        XCTAssertGreaterThanOrEqual(results.count, 1, "Should find record with array skipped in compound index")
