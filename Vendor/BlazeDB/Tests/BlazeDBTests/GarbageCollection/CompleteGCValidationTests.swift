...
final class CompleteGCValidationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "complete_gc_test", fileURL: tempURL, password: "CompleteGCValidation123!")
    }
    ...
    func testCompleteGC_PreventStorageBlowup() throws {
        ...
        // Simulate 3 months of heavy usage (reduced from 6 for faster test execution)
        print("\n🔄 Simulating 3 months of heavy usage...")
        ...
        for month in 1...3 {
            ...
            // Insert 500 records using batch insert for better performance
            print("   📝 Inserting 500 records...")
            let recordsToInsert = (0..<500).map { i in
                BlazeDataRecord([
                    "month": .int(month),
                    "index": .int(i),
            ...
            let newIds = try db.insertMany(recordsToInsert)
            ids.append(contentsOf: newIds)
            
            // Update 250 records (creates new versions, reduced from 500)
            print("   ✏️  Updating 250 records...")
            for id in ids.prefix(250).shuffled() {
                try db.update(id: id, with: BlazeDataRecord([
                    "updated": .bool(true)
                ]))
            }
            
            // Delete 150 old records (reduced from 300)
            print("   🗑️ Deleting 150 old records...")
            if ids.count > 150 {
        ...
        // Even after 3 months of heavy usage, file should not 10x
    ...
    func testGC_RealWorldSimulation() throws {
        ...
        for month in 1...12 {
            ...
            let monthIDs = try db.insertMany(records)
            ids.append(contentsOf: monthIDs)
            totalOperations += monthIDs.count
