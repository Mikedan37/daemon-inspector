...
final class PropertyBasedTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            db = try BlazeDBClient(name: "prop_test", fileURL: tempURL, password: "PropertyBasedTest123!")
        } catch {
    ...
    func testProperty_InsertOrderIndependence() throws {
        ...
        BlazeDBClient.clearCachedKey()
        let db1 = try BlazeDBClient(name: "prop1", fileURL: db1URL, password: "PropertyBasedTest123!")
        ...
        BlazeDBClient.clearCachedKey()
        let db2 = try BlazeDBClient(name: "prop2", fileURL: db2URL, password: "PropertyBasedTest123!")
    ...
    func testProperty_ConcurrentSafety() throws {
        ...
        // This prevents deadlocks and excessive lock contention
        let maxConcurrency = 50
        let semaphore = DispatchSemaphore(value: maxConcurrency)
        
        // 1000 total operations, but only maxConcurrency at a time
        for i in 0..<1000 {
            group.enter()
            ...
            DispatchQueue.global().async {
                defer { 
                    ...
                    if completedCount % 100 == 0 {
                        print("  Completed \(completedCount)/1000 operations...")
                    }
                    progressLock.unlock()
                }
        ...
        // Wait with timeout (120 seconds should be enough for 1000 operations with limited concurrency)
        let timeout = DispatchTime.now() + .seconds(120)
        let result = group.wait(timeout: timeout)
        
        if result == .timedOut {
            XCTFail("Concurrent operations test timed out after 120 seconds. Completed: \(completedCount)/1000")
            return
        }
