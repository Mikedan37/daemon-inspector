...
final class AggregationTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            db = try BlazeDBClient(name: "agg_test_\(testID)", fileURL: tempURL, password: "AggregationTest123!")
        } catch {
    ...
    func testCount() throws {
        // Insert test data using batch insert for speed
    ...
    func testSumWithDouble() throws {
        // Use batch insert for speed
        let records = [
            BlazeDataRecord(["price": .double(10.5)]),
            BlazeDataRecord(["price": .double(20.25)]),
            BlazeDataRecord(["price": .double(5.75)])
        ]
        _ = try db.insertMany(records)
    ...
    func testMin() throws {
        // Use batch insert for speed
        let records = [
            BlazeDataRecord(["value": .int(10)]),
            BlazeDataRecord(["value": .int(5)]),
            BlazeDataRecord(["value": .int(20)])
        ]
        _ = try db.insertMany(records)
    ...
    func testGroupByWithSum() throws {
        // Use batch insert for speed
        let records = [
            BlazeDataRecord(["team": .string("A"), "hours": .double(10.5)]),
            BlazeDataRecord(["team": .string("A"), "hours": .double(20.0)]),
            BlazeDataRecord(["team": .string("B"), "hours": .double(15.0)]),
            BlazeDataRecord(["team": .string("B"), "hours": .double(25.5)])
        ]
        _ = try db.insertMany(records)
    ...
    func testAggregationWithNilValues() throws {
        ...
        let records = [
            BlazeDataRecord(["value": .int(10)]),
            BlazeDataRecord(["other": .string("no value field")]),
            BlazeDataRecord(["value": .int(20)])
        ]
        _ = try db.insertMany(records)
        print("  ✅ Inserted 3 records via batch insert")
    ...
    func testMixedIntAndDouble() throws {
        ...
        let records = [
            BlazeDataRecord(["value": .int(10)]),
            BlazeDataRecord(["value": .double(20.5)])
        ]
        _ = try db.insertMany(records)
    ...
    func testDateMinMax() throws {
        ...
        // Use batch insert for speed
        let records = [
            BlazeDataRecord(["created_at": .date(now)]),
            BlazeDataRecord(["created_at": .date(past)]),
            BlazeDataRecord(["created_at": .date(future)])
        ]
        _ = try db.insertMany(records)
        ...
        if let earliestField = result["earliest"] {
            ...
            case .vector: print("  Case: .vector ❌ WRONG TYPE! Should be .date")
            case .null: print("  Case: .null ❌ WRONG TYPE! Should be .date")
            }
        } else {
    ...
    func testComplexAggregation() throws {
        // Real-world scenario: Bug tracker stats - use batch insert for speed
    ...
    func testBugTrackerDashboard() throws {
        // Simulate real bug tracker data - use batch insert for speed
    ...
    func testTimeSeries() throws {
        ...
        let records = (0..<30).map { i -> BlazeDataRecord in
