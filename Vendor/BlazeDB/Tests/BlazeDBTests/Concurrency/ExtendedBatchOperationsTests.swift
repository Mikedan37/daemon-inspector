...
final class ExtendedBatchOperationsTests: XCTestCase {
    ...
    func testDeleteManyByIDs_HandlesNonExistent() async throws {
        ...
        // Delete is idempotent - returns count of IDs processed, not just existing records
        XCTAssertEqual(deleted, 3, "Should process all IDs (delete is idempotent)")
        
        let remaining = try await db.count()
        XCTAssertEqual(remaining, 3, "Should have 3 records remaining (5 inserted - 2 deleted)")
        
        print("  ✅ Processed 3 deletes (2 existing + 1 non-existent, idempotent)")
    }
