...
final class BlazeDBEnhancedConcurrencyTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        // Small delay to ensure previous database is fully closed
        Thread.sleep(forTimeInterval: 0.05)
        
        // Create unique database file per test run
        let testID = UUID().uuidString
        tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("BlazeConcurrency-\(testID).blazedb")
        ...
        // Also clean up transaction backup files from parent directory
        ...
        db = try BlazeDBClient(name: "ConcurrencyTest_\(testID)", fileURL: tempURL, password: "ConcurrencyTestPassword456!@#")
        
        // Verify database starts empty
        let startCount = try db.count()
        if startCount != 0 {
            // Force cleanup if database is not empty
        ...
        for _ in 0..<100 {
            queue.async {
                ...
                do {
                    ...
                    // Log the error but still fulfill expectation to avoid timeout
                    // Some failures are expected in highly concurrent scenarios
                    print("⚠️ Update failed for record \(randomID.uuidString.prefix(8)): \(error)")
                    expectation.fulfill()
                }
            }
        }
    ...
    func testIndexUpdateRaceCondition() throws {
        ...
        db = nil
        db = try BlazeDBClient(name: "ConcurrencyTest", fileURL: tempURL, password: "ConcurrencyTestPassword456!@#")
