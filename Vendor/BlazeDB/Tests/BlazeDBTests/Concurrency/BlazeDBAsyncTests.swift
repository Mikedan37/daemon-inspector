...
final class BlazeDBAsyncTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "TestAsync", fileURL: tempURL, password: "AsyncTestPassword123!")
    }
    ...
    func testAsyncInsert() async throws {
        ...
        XCTAssertEqual(try fetched?.string("title"), "Test")
    }
    ...
    func testAsyncFetch() async throws {
        ...
        let fetched = try await db.fetchAsync(id: id)
        XCTAssertNotNil(fetched)
        XCTAssertEqual(try fetched?.string("title"), "Async Fetch Test")
        XCTAssertEqual(try fetched?.int("value"), 100)
    }
    ...
    func testAsyncFetchAll() async throws {
        ...
        for i in 0..<20 {
            ...
            _ = try await db.insertAsync(record)
        }
    ...
    func testAsyncFetchPage() async throws {
        ...
        XCTAssertNotEqual(try page1.first?.uuid("id"), try page2.first?.uuid("id"))
    }
    ...
    func testAsyncFetchConcurrent() async throws {
        ...
        for i in 0..<50 {
            ...
            ids.append(try await db.insertAsync(record))
        }
    ...
    func testAsyncUpdate() async throws {
        ...
        let fetched = try await db.fetchAsync(id: id)
        XCTAssertEqual(try fetched?.string("title"), "Updated")
        XCTAssertEqual(try fetched?.int("value"), 2)
    }
    ...
    func testAsyncUpdateConcurrent() async throws {
        ...
        let fetched = try await db.fetchAsync(id: id)
        XCTAssertNotNil(fetched)
        let counter = (try? fetched?.int("counter")) ?? -1
    ...
    func testQueryCacheHit() async throws {
        ...
        XCTAssertEqual(try results1.map { try $0.uuid("id") }, try results2.map { try $0.uuid("id") })
    ...
    func testQueryCacheManualInvalidation() async throws {
        ...
        _ = try await db.insertAsync(newRecord)
    ...
    func testAsyncVsSyncPerformance() async throws {
        ...
        // Sync insert
        let syncStart = Date()
        let syncIds = try await db.insertManyAsync(records)
        let syncDuration = Date().timeIntervalSince(syncStart)
        
        // Clear and try async
        let allRecords = try await db.fetchAllAsync()
        for record in allRecords {
            if let id = record.uuidOptional("id") {
                try await db.deleteAsync(id: id)
            }
        }
