...
final class MVCCIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "mvcc_integ_test", fileURL: tempURL, password: "MVCCIntegrationTest123!")
    }
    ...
    func testMVCC_DataIntegrity() throws {
        ...
        // Final persist to ensure all records are written to disk
        try db.persist()
        
        print("  Inserted and persisted 100 records")
        
        // Verify records are readable before concurrent test
        let verifyCount = try db.count()
        print("  Verified count: \(verifyCount)")
        XCTAssertEqual(verifyCount, 100, "Should have 100 records")
        
        // CRITICAL: Verify each record BEFORE concurrent test
        print("  Verifying each record sequentially...")
        var preTestFailures = 0
        for (id, expected) in expectedRecords {
            if let fetched = try? db.fetch(id: id) {
                let fetchedIndex = fetched.storage["index"]?.intValue
                let expectedIndex = expected.storage["index"]?.intValue
                if fetchedIndex != expectedIndex {
                    print("    ❌ PRE-TEST CORRUPTION: ID \(id.uuidString.prefix(8)) expected[\(expectedIndex ?? -1)], got[\(fetchedIndex ?? -1)]")
                    preTestFailures += 1
                }
            } else {
                print("    ❌ PRE-TEST MISSING: ID \(id.uuidString.prefix(8))")
                preTestFailures += 1
            }
        }
        
        print("  Pre-test verification: \(preTestFailures) failures")
        
        if preTestFailures > 0 {
            XCTFail("Data corruption detected BEFORE concurrent reads! \(preTestFailures) records corrupted during sequential inserts.")
            return
        }
        ...
        for (id, expected) in expectedRecords {
            ...
            DispatchQueue.global().async {
                ...
                if let fetched = try? self.db.fetch(id: id) {
                    // Verify data matches (use .storage subscript)
                    let fetchedIndex = fetched.storage["index"]?.intValue
                    let expectedIndex = expected.storage["index"]?.intValue
                    let fetchedData = fetched.storage["data"]?.stringValue
                    let expectedData = expected.storage["data"]?.stringValue
                    
                    if fetchedIndex != expectedIndex || fetchedData != expectedData {
                        lock.lock()
                        integrityFailures += 1
                        print("    ❌ Mismatch for ID \(id.uuidString.prefix(8)): expected[\(expectedIndex ?? -1), \"\(expectedData ?? "nil")\"], got[\(fetchedIndex ?? -1), \"\(fetchedData ?? "nil")\"]")
                        lock.unlock()
                    }
                } else {
                    lock.lock()
                    integrityFailures += 1
                    print("    ❌ Failed to fetch ID \(id.uuidString.prefix(8))")
                    lock.unlock()
                }
            }
        }
