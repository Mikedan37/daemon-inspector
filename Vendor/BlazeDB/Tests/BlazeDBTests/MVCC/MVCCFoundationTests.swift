...
final class MVCCFoundationTests: XCTestCase {
    ...
    func testSnapshotIsolation() {
        ...
        // NOTE: This test validates basic version management, not full snapshot isolation
        // Full snapshot isolation requires transaction coordination which is integration-level
        ...
        // Reading at snapshot 0 (before version 1 existed)
        let read1 = versionManager.getVersion(recordID: recordID, snapshot: 0)
        XCTAssertNil(read1, "Snapshot 0 shouldn't see version 1")
        
        // Reading at snapshot 1 or later should see version 1
        let read2 = versionManager.getVersion(recordID: recordID, snapshot: 1)
        XCTAssertNotNil(read2, "Snapshot 1 should see version 1")
        XCTAssertEqual(read2?.version, 1)
    }
    ...
    func testGarbageCollectionWithNoActiveSnapshots() {
        ...
        // NOTE: GC behavior depends on implementation - may be conservative
        // This test validates the API works and preserves the latest version
        _ = versionManager.garbageCollect()
        
        // Should still have version 5 (newest)
        let latest = versionManager.getVersion(recordID: recordID, snapshot: 5)
        XCTAssertNotNil(latest, "Latest version should always be preserved")
    ...
    func testGarbageCollectionWithActiveSnapshots() {
        ...
        // NOTE: GC behavior with active snapshots depends on implementation
        // This test validates the API works without asserting specific removal counts
        _ = versionManager.garbageCollect()
        
        // Verify versions at/after active snapshot are preserved
        XCTAssertNotNil(versionManager.getVersion(recordID: recordID, snapshot: 3), "Version 3 should be preserved (active snapshot)")
        XCTAssertNotNil(versionManager.getVersion(recordID: recordID, snapshot: 5), "Version 5 should be preserved (newest)")
