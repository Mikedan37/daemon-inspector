...
final class AutoMigrationVerificationTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            db = try BlazeDBClient(name: "migration_test", fileURL: tempURL, password: "MigrationTest123!")
        } catch {
    ...
    func testMetadataHasEncodingFormat() throws {
        ...
        // With SecureLayout, encodingFormat is nested inside "layout"
        if let layout = json["layout"] as? [String: Any] {
            XCTAssertNotNil(layout["encodingFormat"], "Metadata should have encodingFormat field")
            
            let format = layout["encodingFormat"] as? String
            XCTAssertTrue(format == "json" || format == "blazeBinary", 
                         "encodingFormat should be 'json' or 'blazeBinary', got: \(format ?? "nil")")
        } else {
            // Fallback: check top-level (for non-secure layouts)
    ...
    func testAllRecordsSurviveMigration() throws {
        ...
        // With SecureLayout, encodingFormat is nested inside "layout"
        if var layout = json["layout"] as? [String: Any] {
            print("🔍 Before update: layout.encodingFormat = \(layout["encodingFormat"] ?? "nil")")
            layout["encodingFormat"] = "json"  // ✅ Mark as JSON format (to trigger migration)
            json["layout"] = layout
            print("🔍 After update: layout.encodingFormat = \(layout["encodingFormat"] ?? "nil")")
        } else {
            // Fallback: check top-level (for non-secure layouts)
        ...
        if let layout = verifyJson["layout"] as? [String: Any] {
            print("✅ Metadata encodingFormat verified on disk: \(layout["encodingFormat"] ?? "nil")")
        } else {
        ...
        for (id, expectedIndex) in expectedValues {
            ...
            if record != nil {
                ...
                if failCount == 1 {
                    ...
                    print("   Page number: \(db.collection.indexMap[id]?.first ?? -1)")
                }
            }
        }
    ...
    func testEncodingFormatConsistent() throws {
        ...
        for _ in 0..<5 {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                ...
                let format: String?
                if let layout = json["layout"] as? [String: Any] {
                    format = layout["encodingFormat"] as? String
                } else {
                    // Fallback: check top-level (for non-secure layouts)
                    format = json["encodingFormat"] as? String
                }
    ...
    func testLargeDatasetMigration() throws {
        // Insert 1000 records using insertMany for better performance
    ...
    func testMigrationFailurePreservesData() throws {
        ...
        db = nil
        BlazeDBClient.clearCachedKey()
        let backupDB2 = try BlazeDBClient(name: "backup_test", fileURL: backupDB, password: "MigrationTest123!")
