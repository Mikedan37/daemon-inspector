...
final class BlazeDBInitializationTests: XCTestCase {
    ...
    func testThrowingInit_WeakPassword() throws {
        ...
        XCTAssertThrowsError(try BlazeDBClient(
            name: "TestDB",
            fileURL: tempURL,
            password: "weak"  // Only 4 characters, no uppercase, no numbers, no special chars
        )) { error in
            XCTAssertTrue(error is BlazeDBError, "Should throw BlazeDBError")
            if case BlazeDBError.transactionFailed(let msg, _) = error {
                XCTAssertTrue(msg.contains("12") || msg.contains("uppercase") || msg.contains("character"), "Should mention password requirements")
                XCTAssertTrue(msg.contains("weak") || msg.contains("Password"), "Should mention password issue")
            }
        }
    ...
    func testFailableInit_Success() throws {
        ...
        var db: BlazeDBClient? = try? BlazeDBClient(name: "TestDB", fileURL: tempURL, password: "SecurePassword123!")
    ...
    func testFailableInit_WeakPassword() throws {
        ...
        let db = try? BlazeDBClient(name: "TestDB", fileURL: tempURL, password: "weak")
    ...
    func testFailableInit_InvalidPath() throws {
        ...
        // Should return nil (not crash)
        let db = try? BlazeDBClient(name: "TestDB", fileURL: invalidURL, password: "SecurePassword123!")
    ...
    func testFailableInit_VariousWeakPasswords() throws {
        ...
        for password in weakPasswords {
            let db = try? BlazeDBClient(name: "TestDB", fileURL: tempURL, password: password)
    ...
    func testFailableInit_MinimumValidPassword() throws {
        print("🔷 Testing failable initializer with minimum valid password (12 chars, mixed case, numbers, special)...")
        
        var db: BlazeDBClient? = try? BlazeDBClient(name: "TestDB", fileURL: tempURL, password: "MinPass123!X")
        
        guard let dbUnwrapped = db else {
            XCTFail("Should accept minimum valid password")
            return
        }
        ...
        print("✅ Failable initializer accepts minimum valid password")
    }
    ...
    func testErrorMessages_AreHelpful() throws {
        ...
        XCTAssertTrue(errorMessage.contains("12") || errorMessage.contains("character") || errorMessage.contains("uppercase"), 
                     "Should mention character requirement or complexity requirements")
    ...
    func testFailableInit_LogsErrors() throws {
        ...
        // CRITICAL: Use failable initializer (not throwing) to test error logging
        // The failable init logs "Failed to initialize", the throwing init just throws
    ...
    func testRealWorld_GuardLetPattern() throws {
        ...
        func initializeDatabase() -> BlazeDBClient? {
            guard let dbInstance = try? BlazeDBClient(
                name: "ProductionDB",
                fileURL: tempURL,
    ...
    func testRealWorld_ErrorRecovery() throws {
        ...
        db = try? BlazeDBClient(name: "TestDB", fileURL: tempURL, password: "weak")
        XCTAssertNil(db, "First attempt should fail")
        
        // Retry with strong password (simulating retry logic)
        db = try? BlazeDBClient(name: "TestDB", fileURL: tempURL, password: "StrongPassword123!")
