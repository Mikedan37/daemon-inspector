...
final class BlazeBinaryPerformanceTests: XCTestCase {
    ...
    func testPerformance_Encode_BlazeBinary() {
        ...
        measure(metrics: [XCTClockMetric()]) {
            for _ in 0..<1000 {
                // UPDATED: Use ARM codec for performance tests (it's the optimized path)
    ...
    func testBenchmark_StorageSize_SimpleRecord() throws {
        ...
        // UPDATED: Use ARM codec (both produce identical sizes)
        let binarySize = try BlazeBinaryEncoder.encodeARM(record).count
    ...
    func testBenchmark_StorageSize_10kRecords() throws {
        ...
        for i in 0..<10000 {
            ...
            // UPDATED: Use ARM codec (both produce identical sizes)
            binaryTotal += try BlazeBinaryEncoder.encodeARM(record).count
        }
