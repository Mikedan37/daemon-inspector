...
final class BlazeBinaryUltimateBulletproofTests: XCTestCase {
    ...
    func testUltimate_SimulateMemoryCorruption_BitFlips() throws {
        ...
        // Enable CRC32 to detect bit flips anywhere in the data
        let originalCRC32Mode = BlazeBinaryEncoder.crc32Mode
        BlazeBinaryEncoder.crc32Mode = .enabled
        defer { BlazeBinaryEncoder.crc32Mode = originalCRC32Mode }
        ...
        // UPDATED: Use dual-codec validation and deterministic corruption
        // Encode with CRC32 enabled for corruption detection
        let validEncoding = try BlazeBinaryEncoder.encodeARM(record)
        
        // Simulate 100 deterministic bit flips (using fixed seed for reproducibility)
        var detectedCount = 0
        var seed: UInt64 = 42
        ...
        for i in 0..<100 {
            ...
            // Deterministic byte selection
            let randomByte = Int(seed % UInt64(corrupted.count))
            seed = seed &* 1103515245 &+ 12345
            // Deterministic bit selection
            let randomBit = UInt8(1 << Int(seed % 8))
            
            // Flip bit
            corrupted[randomByte] ^= randomBit
            
            // UPDATED: Both codecs must detect corruption identically
            var stdDetected = false
            var armDetected = false
            
            do {
                _ = try BlazeBinaryDecoder.decode(corrupted)
            } catch {
                stdDetected = true
            }
            
            do {
                _ = try BlazeBinaryDecoder.decodeARM(corrupted)
            } catch {
                armDetected = true
            }
            
            // Both codecs should have same detection behavior
            XCTAssertEqual(stdDetected, armDetected, "Both codecs must detect corruption identically for corruption \(i)")
            
            if stdDetected || armDetected {
                detectedCount += 1
            }
        }
