...
final class BlazeBinaryDirectVerificationTests: XCTestCase {
    ...
    func testDirect_NoDataCorruption_InEncodeDecodeChain() throws {
        ...
        for (index, value) in criticalValues.enumerated() {
            ...
            case .vector(let expected):
                if case .vector(let decodedVector) = decodedValue {
                    XCTAssertEqual(decodedVector.count, expected.count, "Vector \(index) corrupted!")
                } else {
                    XCTFail("Vector \(index) corrupted! Expected vector, got \(String(describing: decodedValue))")
                }
            case .null:
                XCTAssertNil(decodedValue, "Null \(index) corrupted!")
            }
        }
