...
final class BlazeDBStressTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "StressTest", fileURL: tempURL, password: "StressTestPassword123!")
    }
    ...
    func test100ConcurrentWriters() throws {
        ...
        for writerID in 0..<writerCount {
            ...
            queue.async {
                ...
                // PERFORMANCE: Batch all records for this writer together
                // This reduces lock contention from 5000 individual inserts to 100 batch inserts
                // Each writer inserts its 50 records in a single batch operation
                let records = (0..<recordsPerWriter).map { i in
                    BlazeDataRecord([
                        "writer": .int(writerID),
                        "index": .int(i),
                        "timestamp": .date(Date())
                ...
                do {
                    let insertedIDs = try self.db.insertMany(records)
                    
                    successLock.lock()
                    successCount += insertedIDs.count
                    successLock.unlock()
                } catch {
                    errorLock.lock()
                    errors.append(error)
