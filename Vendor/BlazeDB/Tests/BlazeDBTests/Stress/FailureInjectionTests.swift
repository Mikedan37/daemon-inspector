...
final class FailureInjectionTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            db = try BlazeDBClient(name: "fail_test", fileURL: tempURL, password: "FailureInjectionTest123!")
        } catch {
    ...
    func testCorruptedMetadataRecovery() throws {
        ...
        do {
            self.db = try BlazeDBClient(name: "fail_test", fileURL: self.tempURL, password: "FailureInjectionTest123!")
            XCTAssertNotNil(db, "Should handle corrupted metadata gracefully and initialize database")
        } catch {
            XCTFail("Should handle corrupted metadata gracefully, but got error: \(error)")
        }
    }
    ...
    func testMissingMetadataRecovery() throws {
        ...
        do {
            self.db = try BlazeDBClient(name: "fail_test", fileURL: self.tempURL, password: "FailureInjectionTest123!")
            XCTAssertNotNil(db, "Should handle missing metadata gracefully and initialize database")
        } catch {
            XCTFail("Should handle missing metadata gracefully, but got error: \(error)")
        }
    }
    ...
    func testWrongPasswordFailsGracefully() throws {
        // Insert with password "FailureInjectionTest123!"
        for i in 0..<10 {
            try db.insert(BlazeDataRecord(["value": .int(i)]))
        }
        try db.persist()
        ...
        // Try to open with wrong password (but still meets strength requirements)
        let wrongPasswordDB = try BlazeDBClient(name: "fail_test", fileURL: tempURL, password: "WrongPassword123!")
    ...
    func testDetectPartialMetadataWrite() throws {
        ...
        do {
            ...
            XCTFail("Should handle partial metadata write gracefully, but got error: \(error)")
        }
    }
}
