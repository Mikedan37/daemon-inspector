...
final class BlazeDBPerformanceTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        db = try BlazeDBClient(name: "PerfTest", fileURL: tempURL, password: "BlazeDBPerformanceTest123!")
    }
    ...
    func testIndexQueryPerformance() throws {
        // Scoped initialization to ensure proper cleanup before reopening
        do {
            ...
            print("🔵 Starting insert of 300 records...")
            ...
            db = nil  // Close within scope
        }
        
        print("🔵 Reopening database...")
        // Reopen after scope ensures deinitialization
        db = try BlazeDBClient(name: "PerfTest", fileURL: tempURL, password: "BlazeDBPerformanceTest123!")
        let rebuiltCollection = db.collection
        print("✅ Reopened!")
        
        // PERFORMANCE: Pre-warm the index by doing one query before measuring
        // This ensures the index is rebuilt (if needed) before the performance measurement
        print("🔵 Pre-warming index...")
