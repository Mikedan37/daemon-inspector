...
final class EncryptionSecurityTests: XCTestCase {
    ...
    // NOTE: Different passwords test consolidated into EncryptionSecurityFullTests.swift (testSecurity_DifferentPasswordsDifferentEncryption)
    // which tests ciphertext differences more directly
    ...
    // NOTE: Data encrypted on disk test moved to EncryptionSecurityFullTests.swift (testSecurity_DataEncryptedOnDisk)
    // to avoid duplication
    
    // NOTE: Multiple databases test consolidated into EncryptionSecurityFullTests.swift (testSecurity_DifferentPasswordsDifferentEncryption)
    // which tests the same concept with ciphertext comparison
