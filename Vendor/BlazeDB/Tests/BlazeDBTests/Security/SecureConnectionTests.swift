...
final class SecureConnectionTests: XCTestCase {
    ...
    func testKeyDerivation() throws {
        ...
        // Convert SharedSecret to SymmetricKey (matching SecureConnection.swift pattern)
        let clientSecretKey = SymmetricKey(data: clientSecret.withUnsafeBytes { Data($0) })
        let serverSecretKey = SymmetricKey(data: serverSecret.withUnsafeBytes { Data($0) })
        
        let clientKey = HKDF<SHA256>.deriveKey(
            inputKeyMaterial: clientSecretKey,
            salt: salt,
            info: info,
            outputByteCount: 32
        )
        ...
        let serverKey = HKDF<SHA256>.deriveKey(
            inputKeyMaterial: serverSecretKey,
            salt: salt,
            info: info,
            outputByteCount: 32
        )
        ...
        // Keys should match
        let clientKeyData = clientKey.withUnsafeBytes { Data($0) }
        let serverKeyData = serverKey.withUnsafeBytes { Data($0) }
        XCTAssertEqual(clientKeyData, serverKeyData)
    }
