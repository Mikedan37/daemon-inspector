...
final class DistributedGCTests: XCTestCase {
    ...
    func testOperationLogGC_FullCleanup() async throws {
        ...
        config.keepLastOperationsPerRecord = 100
    ...
    func testSyncStateGC_CleanupDeletedRecords() async throws {
        ...
        // Insert some records
        let id1 = try await db1.insert(BlazeDataRecord(["value": .int(1)]))
        let id2 = try await db1.insert(BlazeDataRecord(["value": .int(2)]))
        let id3 = try await db1.insert(BlazeDataRecord(["value": .int(3)]))
        
        // Simulate sync state (manually set for testing)
        // In real usage, this would be set during sync
        
        // Delete one record
        try await db1.delete(id: id2)
    ...
    func testSyncStateGC_FullCleanup() async throws {
        ...
        var config = SyncStateGCConfig()
        config.retentionDays = 7
        config.cleanupInterval = 3600
    ...
    func testIntegration_OperationLogGCWithSync() async throws {
        ...
        // Insert records
        let id = try await db1.insert(BlazeDataRecord(["value": .int(1)]))
    ...
    func testIntegration_SyncStateGCWithMultipleRecords() async throws {
        ...
        for i in 5..<10 {
            let synced = try await db2.fetch(id: ids[i])
