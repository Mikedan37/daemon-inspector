...
final class UnixDomainSocketTests: XCTestCase {
    ...
    func testBlazeBinaryEncoding_MultipleOperations() throws {
        ...
        // Decode all operations (use loadUnaligned to avoid alignment crashes)
        var offset = 0
        ...
        for (i, op) in decodedOps.enumerated() {
            XCTAssertEqual(op.changes["index"]?.intValue, i)
        }
    }
    ...
    func testCrossAppSync_UnixDomainSocket() async throws {
        ...
        let db1 = try BlazeDBClient(name: "App1DB", fileURL: db1URL, password: "UnixDomainSocketTest123!")
        let db2 = try BlazeDBClient(name: "App2DB", fileURL: db2URL, password: "UnixDomainSocketTest123!")
        ...
        XCTAssertEqual(try synced?.string("message"), "Hello from App1!")
    ...
    func testCrossAppSync_Bidirectional() async throws {
        ...
        let db1 = try BlazeDBClient(name: "App1", fileURL: db1URL, password: "UnixDomainSocketTest123!")
        let db2 = try BlazeDBClient(name: "App2", fileURL: db2URL, password: "UnixDomainSocketTest123!")
        ...
        // Insert in both apps
        let id1_record = try await db1.insert(BlazeDataRecord(["source": .string("app1"), "value": .int(1)]))
        let id2_record = try await db2.insert(BlazeDataRecord(["source": .string("app2"), "value": .int(2)]))
        ...
        XCTAssertNotNil(fetched1, "App1's record should be in App2")
        let fetched2 = try await db1.fetch(id: id2_record)
        XCTAssertNotNil(fetched2, "App2's record should be in App1")
    }
    ...
    func testCrossAppSync_Performance() async throws {
        ...
        var recordIds: [UUID] = []
        for i in 0..<1000 {
            let id = try await db1.insert(BlazeDataRecord([
                "index": .int(i),
