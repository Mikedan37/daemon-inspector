...
final class SyncEndToEndTests: XCTestCase {
    ...
    func testLocalSyncEndToEnd() async throws {
        ...
        XCTAssertEqual(synced?.string("title"), "Test Record")
    ...
    func testUpdateSyncEndToEnd() async throws {
        ...
        // Update in db1
        var updated = try db1.fetch(id: recordId)!
        updated.storage["title"] = .string("Updated")
        updated.storage["value"] = .int(2)
        try db1.update(id: recordId, with: updated)
        ...
        let synced = try db2.fetch(id: recordId)
        XCTAssertEqual(synced?.string("title"), "Updated")
        XCTAssertEqual(synced?.int("value"), 2)
    }
    ...
    func testDeleteSyncEndToEnd() async throws {
        ...
        XCTAssertNotNil(try db2.fetch(id: recordId))
        
        // Delete in db1
        try db1.delete(id: recordId)
        
        // Wait for delete sync
        try await Task.sleep(nanoseconds: 500_000_000)
        
        // Verify deleted in db2
        XCTAssertNil(try db2.fetch(id: recordId))
    }
    ...
    func testBidirectionalSyncEndToEnd() async throws {
        ...
        // Insert in db1
        let id1_record = try db1.insert(BlazeDataRecord([
            "source": .string("db1"),
            "value": .int(1)
        ]))
        
        // Insert in db2
        let id2_record = try db2.insert(BlazeDataRecord([
            "source": .string("db2"),
            "value": .int(2)
        ]))
        ...
        XCTAssertNotNil(try db2.fetch(id: id1_record), "db1's record should be in db2")
        XCTAssertNotNil(try db1.fetch(id: id2_record), "db2's record should be in db1")
    }
}
