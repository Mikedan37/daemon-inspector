...
final class SyncIntegrationTests: XCTestCase {
    ...
    func testMemorySafety_ConcurrentOperations() async throws {
        ...
        let db1Count = try db1.query().all().count
        let db2Count = try db2.query().all().count
    ...
    func testPerformance_LargeBatchSync() async throws {
        ...
        let insertIds = try db1.insertMany(records)
    ...
    func testEdgeCase_RapidConnectDisconnect() async throws {
        ...
        // Insert a record
        let recordId = try db1.insert(BlazeDataRecord(["test": .string("value")]))
    ...
    func testEdgeCase_LargeRecords() async throws {
        ...
        XCTAssertEqual(synced?.string("largeData"), largeString, "Large data should match")
    }
        ...
        await withTaskGroup(of: Void.self) { group in
            for _ in 0..<100 {
                group.addTask {
                    ...
                    _ = try? await topology.getNode(id: id1)
                    _ = try? await topology.getNode(id: id2)
                }
            }
        }
        
        // Should not crash (actor prevents data races)
        let node1 = try await topology.getNode(id: id1)
        let node2 = try await topology.getNode(id: id2)
