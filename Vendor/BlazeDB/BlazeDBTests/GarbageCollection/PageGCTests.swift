...
final class PageGCTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        db = try BlazeDBClient(name: "page_gc_test", fileURL: tempURL, password: "test-pass-123")
    }
    ...
    func testVACUUM_ReclaimsSpace() throws {
        ...
        try db.persist()
        let sizeBeforeDelete = try tempURL.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0
    ...
    func testAutoVacuum_TriggersWhenNeeded() throws {
        ...
        let sizeBefore = try tempURL.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0
        
        // Auto-vacuum should trigger
        try db.autoVacuumIfNeeded()
        
        let sizeAfter = try tempURL.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0
