...
final class PageStoreBoundaryTests: XCTestCase {
    ...
    /// Write/read exactly (pageSize - 9) bytes — the max payload once header (4) + version (1) + length (4) are accounted for.
    func testMaxPayloadRoundTrip() throws {
        let url = tmpURL()
        let key = SymmetricKey(size: .bits256)
        let pageSize = try inferPageSize(at: url, key: key)

        let maxPayload = pageSize - 9
    ...
    /// Attempt to write (pageSize - 8) bytes — should fail because it would overflow the page.
    func testTooLargePayloadThrows() throws {
        let url = tmpURL()
        let key = SymmetricKey(size: .bits256)
        let pageSize = try inferPageSize(at: url, key: key)

        let tooLarge = pageSize - 8
