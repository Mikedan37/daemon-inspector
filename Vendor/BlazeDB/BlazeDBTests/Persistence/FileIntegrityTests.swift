...
final class FileIntegrityTests: XCTestCase {
    ...
    override func setUp() {
        ...
        db = try! BlazeDBClient(name: "file_test", fileURL: tempURL, password: "test-pass-123")
    }
    ...
    func testMetadataAlwaysValidJSON() throws {
        ...
        let stages: [(String, () throws -> Void)] = [
            ...
            ("After reopen", {
                self.db = nil
                BlazeDBClient.clearCachedKey()
                self.db = try BlazeDBClient(name: "file_test", fileURL: self.tempURL, password: "test-pass-123")
            })
        ]
