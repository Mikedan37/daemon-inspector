...
final class PersistenceIntegrityTests: XCTestCase {
    ...
    override func setUp() {
        ...
        db = try! BlazeDBClient(name: "persist_test", fileURL: tempURL, password: "test-pass-123")
    }
    ...
    func testMetadataRemainsValidJSON() throws {
        ...
        if let dict = json as? [String: Any] {
            XCTAssertNotNil(dict["indexMap"], "Should have indexMap")
            XCTAssertNotNil(dict["nextPageIndex"], "Should have nextPageIndex")
            XCTAssertNotNil(dict["encodingFormat"], "Should have encodingFormat")
        } else {
